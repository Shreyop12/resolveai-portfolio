          ...
          {tickets.length === 0 ? <p className="empty">Create a workspace, then log the first customer ticket.</p> : (
            <ul>
              {tickets.map((ticket) => <li key={ticket.ticket_id}>
                ...
                {triageByTicket[ticket.ticket_id] && !draftsByTicket[ticket.ticket_id] && <button type="button" className="secondary-button" onClick={() => assessTicket(ticket.ticket_id)} disabled={isActionPending(`assess-ticket:${ticket.ticket_id}`)}>{isActionPending(`assess-ticket:${ticket.ticket_id}`) ? "Reassessing…" : "Reassess ticket"}</button>}
